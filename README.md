...
3. In the repository directory, run `./dev_scripts/setup_dev.sh` to configure git hooks. If on Windows, a popup might appear asking you what tool you want to run the script with; choose Git Bash. Branch names must include a Jira ticket ID (for example `SCRUM-123-my-branch`); commits on those branches are prefixed with the ticket ID automatically.
